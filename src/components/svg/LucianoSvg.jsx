...
export default function LucianoSvg({ colorClass, homeVisible = false }) {
  ...
  useEffect(() => {
    console.log(`SVG STARTED color class is: ${colorClass}`);
  }, []);
  return (
    <svg
      viewBox="0 0 525 139"
      ...
      className={
        homeVisible
          ? // ON THE HOME SECTION
            colorClass +
            " left-[2rem] lg:left-[calc(15%-1.5rem)] top-[calc(50%-5rem-2.5rem)] drop-shadow-[0_10px_10px_#14213d50] lg:top-[calc(50%-7rem-3.5rem)] h-[5rem] lg:h-[7rem] absolute transition-[top_left] ease-in-out duration-1000 z-40"
          : // CENTERED
            colorClass +
            " animate-opacity-fade-in-1s-delay left-[calc(50%-17rem)] top-[calc(50%-4.5rem)] drop-shadow-[0_10px_10px_#14213d50] h-[9rem] absolute transition-[top_left] ease-in-out duration-1000 z-40"
      }
    >
      <path
